...
class StopSignDetector():
	...
	def get_bounding_box(self, img):
		...
		for cont in contours:
			...
			if len(approx) >= 8 and cv2.contourArea(cont) >= 200:#img.shape[0]*img.shape[1]/20: #For each approximated region, if the number of points larger than 8 and the area larger than 200 pixels, then a stop sign is detected.
				x, y, w, h = cv2.boundingRect(approx)
				box = [x, (testImage.shape[0] - y), (x + w), (testImage.shape[0] - y - h)] #Convert the bounding box points in row-column coordinate to x-y coordinate
				boxes.append(box)
		return boxes
...
if __name__ == '__main__':
	...
	for box in boxes:
		cv2.rectangle(img, (box[0], img.shape[0] - box[1]), (box[2], img.shape[0] - box[3]), (255, 255, 123),2)  # Draw the bounding box on the image
	cv2.imshow('image', img)
	cv2.waitKey(0)
	cv2.destroyAllWindows()
